...
int main()
{
	loaf_service_locator_i api_locator;

	loaf_load_service_locator(&api_locator);

	api_locator.construct(&api_locator);

	module_loader_i * module_loader = create_default_module_loader();

	loaf_taskman_i * swarm = loaf_get_taskman();

	swarm->construct(swarm, 4096, 11);
	...
	for (const auto & entry : std::filesystem::directory_iterator("."))
	{
		if (entry.path().extension() == ".mod")
		{
			...
			std::strcpy(path, entry.path().c_str());
	...
	const char * err = module_loader->load_files(module_loader, module_paths.data(), module_paths.size());
	...
	module_loader->unload_all(module_loader);

	destroy_default_module_loader(module_loader);

	swarm->destruct(swarm);

	api_locator.destruct(&api_locator);

	loaf_unload_service_locator(&api_locator);
